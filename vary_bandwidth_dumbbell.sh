...
# This script runs a series of dumbbell simulations with different TCP variants
...
# set working DIR
if [ ! -e "$New_DIR/tcp-eval-results/dumbbell/bandwidth" ]; then
    mkdir "$New_DIR/tcp-eval-results/dumbbell/bandwidth"
fi

i=0
while true ; do
    if [ ! -e "$New_DIR/tcp-eval-results/dumbbell/bandwidth/exp$i" ]; then
        mkdir  "$New_DIR/tcp-eval-results/dumbbell/bandwidth/exp$i"   
        Working_DIR="$New_DIR/tcp-eval-results/dumbbell/bandwidth/exp$i"
        break
    fi
    let "i=i+1"
done
...
run_a_session ( ) {
    ...
    for tcp_var in TcpNewReno TcpWestwood TcpWestwoodPlus
        do
            for var in $1  
                do
                    ./waf --run "drive-dumbbell --bottleneckBandwidth=$var --tcp_variant=$tcp_var --fileName=$Working_DIR/bandwidth.$tcp_var"
            done
    done
}
