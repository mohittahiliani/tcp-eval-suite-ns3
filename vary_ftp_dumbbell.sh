...
# set working DIR
if [ ! -e "$New_DIR/tcp-eval-results/dumbbell/ftp" ]; then
    mkdir "$New_DIR/tcp-eval-results/dumbbell/ftp"
fi

i=0
while true ; do
    if [ ! -e "$New_DIR/tcp-eval-results/dumbbell/ftp/exp$i" ]; then
        mkdir  "$New_DIR/tcp-eval-results/dumbbell/ftp/exp$i"   
        Working_DIR="$New_DIR/tcp-eval-results/dumbbell/ftp/exp$i"
        break
    fi
    let "i=i+1"
done
...
run_a_session ( ) {
    ...
    for tcp_var in TcpTahoe TcpReno TcpNewReno TcpWestwood TcpWestwoodPlus
        do
            for var in $1  
                do
                    ./waf --run "drive-dumbbell --nFwdFtpFlows=$var --tcp_variant=$tcp_var --fileName=$Working_DIR/ftp.$tcp_var"
            done
    done
}
