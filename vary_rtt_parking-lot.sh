...
# This script runs a series of parking-lot simulations with different 
...
# set working DIR
if [ ! -e "$New_DIR/tcp-eval-results/parking-lot/rtt" ]; then
    mkdir "$New_DIR/tcp-eval-results/parking-lot/rtt"
fi

i=0
while true ; do
    if [ ! -e "$New_DIR/tcp-eval-results/parking-lot/rtt/exp$i" ]; then
        mkdir  "$New_DIR/tcp-eval-results/parking-lot/rtt/exp$i"   
        Working_DIR="$New_DIR/tcp-eval-results/parking-lot/rtt/exp$i"
        break
    fi
    let "i=i+1"
done
...
run_a_session ( ) {
    ...
    for tcp_var in TcpTahoe TcpReno TcpNewReno TcpWestwood TcpWestwoodPlus
        do
            for var in $1  
                do
                    ./waf --run "drive-parking-lot --rttp=$var --tcp_variant=$tcp_var --fileName=$Working_DIR/rttp.$tcp_var"
            done
    done
}
